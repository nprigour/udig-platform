...
public class EditManagerImpl extends EObjectImpl implements EditManager {
    ...
    private void commitRollbackComplete() throws IOException {
        ...
        //HACK this small delay seems to fix
        //the issue causing  constant problems
        //during commit or rollback actions while
        //editing features.
        try {
            Thread.sleep(200);
        } catch (InterruptedException e) {
    ...
    /**
     * QUICK FIX to address problem with SQL Connection being closed
     * for unknown reason.
     * Method used if we catch an exception during 
     * {@link #commitTransaction()} and {@link #rollbackTransaction()}
     */
    private void resetEditLayerConnection() {
        try {
            ...
            IService serviceOriginal =getMapInternal().getEditManagerInternal().getEditLayer().findGeoResource(FeatureStore.class).service(new NullProgressMonitor());
            catalog.replace(serviceOriginal.getID(), serviceFactory.createService(serviceOriginal.getConnectionParams()).get(0));  
